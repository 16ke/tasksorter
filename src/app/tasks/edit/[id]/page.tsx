// src/app/tasks/edit/[id]/page.tsx - WITH CATEGORY CREATION MODAL
"use client";
      ...
      <div className="min-h-screen py-8">
        <div className="max-w-2xl mx-auto p-4">
          <div className="mb-6">
            ...
            <h1 className="text-3xl font-bold mt-2 text-foreground">Loading Task...</h1>
          </div>
          <div className="bg-card p-6 rounded-lg shadow-md border-theme animate-pulse">
      ...
      <div className="min-h-screen py-8">
        <div className="max-w-2xl mx-auto p-4">
          <div className="mb-6">
            ...
            <h1 className="text-3xl font-bold mt-2 text-foreground">Edit Task</h1>
          </div>

          <form onSubmit={handleSubmit} className="space-y-6 bg-card p-6 rounded-lg shadow-md border-theme">
            ...
            <div>
              <div className="flex justify-between items-center mb-3">
                ...
                <button
                  ...
                  className="text-sm bg-[var(--turquoise-500)] text-white px-3 py-1 rounded-md hover:bg-[var(--turquoise-600)] transition-colors"
                >
                  + New Category
                </button>
              </div>
              ...
              {categoriesLoading ? (
                <div className="flex flex-wrap gap-2">
                  {[1, 2, 3].map(n => (
                    <div key={n} className="px-3 py-2 bg-gray-200 dark:bg-gray-700 rounded-full animate-pulse w-20"></div>
                  ))}
                </div>
              ) : categories.length === 0 ? (
                <div className="text-center py-4 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg">
                  <p className="text-muted text-sm mb-2">No categories yet</p>
                  <button
                    type="button"
                    onClick={() => setIsModalOpen(true)}
                    className="text-[var(--turquoise-600)] hover:underline text-sm font-medium"
                  >
                ...
                <div className="flex flex-wrap gap-2">
                  {categories.map((category) => (
                    <button
                      ...
                      className={`px-3 py-2 text-sm font-medium rounded-full border transition-all duration-200 flex items-center space-x-2 ${
                        selectedCategories.includes(category.id)
                          ? 'text-white shadow-lg scale-105'
                          : 'bg-card text-foreground border-theme hover:shadow-md'
                      }`}
                      style={
                        selectedCategories.includes(category.id)
                          ? {
            ...
            <div className="flex space-x-4 pt-4">
              ...
              <button
                ...
                className="flex-1 bg-gray-300 text-gray-700 py-3 px-4 rounded-md hover:bg-gray-400 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 transition-colors dark:bg-gray-600 dark:text-gray-200 dark:hover:bg-gray-500 font-medium"
              >
                Cancel
              </button>
            </div>
